...
create policy "Users can select their own loans" on public.loans for select using (auth.uid() = user_id);
...
create index if not exists habit_pauses_user_habit_idx on public.habit_pauses (user_id, habit_id);
create index if not exists habit_checks_user_day_idx on public.habit_checks (user_id, day desc);
